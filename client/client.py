...
def upload_file_with_grpc(file_content):
    with grpc.insecure_channel("grpc-server:50051") as channel:
        ...
        response = stub.UploadFile(
            iter([file_transfer_pb2.FileChunk(content=file_content)])
        )
        return response.success


def send_text_with_grpc(text):
    with grpc.insecure_channel("grpc-server:50052") as channel:
        stub = send_text_pb2_grpc.SendTextStub(channel)
        request = send_text_pb2.Text(text=text)
...
async def upload_file_rest(file: UploadFile = File(...)):
    server_url = "http://rest-server:8000/upload/"
    headers = {
...
async def send_text_grpc(text: str):
    ...
    if success:
        ...
        return {"message": "Failed to send text"}


@app.post("/send_text_rest")
async def send_text_rest(text: str):
    server_url = "http://rest-server:8000/send_text/"
    headers = {
        "accept": "application/json"
    }  # Set the Content-Type header for file upload
    start_time = time.time()
